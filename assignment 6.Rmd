...
```{r opc2}
maposta2 <- leaflet(subte) %>%
  addProviderTiles(providers$CartoDB) %>%
  addCircles(fillColor = ~pal(Line), stroke = ifelse(subte$Transfer == "Yes", 0.5, FALSE), fillOpacity = 1, color = ifelse(subte$Transfer == "Yes", "black", FALSE), opacity = 0.8, radius = ifelse(subte$Elevator == "Yes", 20, 10),
              highlightOptions = highlightOptions(fillColor = "gray",         fillOpacity = 1),
             popup = ~describe) %>%
  addControl("Subway entrances in Buenos Aires", position = "topright") %>%
  addControl('<a href="https://data.buenosaires.gob.ar/dataset/bocas-subte">Data source</a>',
...
```{r savoir piquetero 2}
saveWidget(maposta2, file = "subte2.html")
```